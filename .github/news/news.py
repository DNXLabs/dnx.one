...
docs_repository = github.repository('DNXLabs', 'dnx.one')
github_docs_contents = docs_repository.directory_contents('src/news/', return_as=dict)

news = []
all_releases = []

count=0
for repository in github.repositories_by('DNXLabs'):

    repository_name = repository.as_dict()['name']
    gh_repository = github.repository('DNXLabs', repository_name)
    ...
    for release in repo_releases:
        ...
        release_dict['repository'] = repository_name
...
for key, value in content.items():
    ...
    filename = os.path.join(root, '../../src/news/%s-%s-01-news.md' % (year, month))
    value.sort(key=lambda item:item['created_at'], reverse=True)
    for vl in value:
        vl['body'] = vl['body'].replace('<br>', '')
    ...
    with open(filename) as f:
        news_file = f.read()
        file_name = '%s-%s-01-news.md' % (year, month)
        if file_name in github_docs_contents:
            github_docs_index = docs_repository.file_contents('/src/news/' + file_name, ref=MAIN_BRANCH)
